<?php
/*
Plugin Name: Revived Ajax Event Calendar
Plugin URI: http://github.com/kokarn/ajax-event-calendar
Description: A revived version of the Ajax Event Calendar
Version: 1.0.0
Author: Oskar Risberg
License: MIT
*/
...
if (!class_exists('ajax_event_calendar')) {
	class ajax_event_calendar {
		...
		function __construct() {
			...
			// basic setup for timezones
			add_filter( 'calendar_timezone', array( $this, 'change_calendar_timezone' ), 10, 3 );
		}
	    ...
	    function localize_plugin($page) {
			...
			// register scripts
			wp_register_script('fullcalendar', 'https://cdnjs.cloudflare.com/ajax/libs/fullcalendar/1.6.4/fullcalendar.min.js', array('jquery'), '1.6.4', true);
		...
		function change_calendar_timezone( $events, $timezone ){
			foreach( $events as $event ):
				date_default_timezone_set( 'UTC' );

				$startTimestamp = strtotime( $event->start );
				$event->UTCStart = $event->start;
				$startTimezoneTimestamp = $startTimestamp - ( $timezone * 60 );
				$event->start = date( 'Y-m-d H:i:s', $startTimezoneTimestamp );

				$endTimestamp = strtotime( $event->end );
				$event->UTCEnd = $event->end;
				$endTimezoneTimestamp = $endTimestamp - ( $timezone * 60 );
				$event->end = date( 'Y-m-d H:i:s', $endTimezoneTimestamp );
			endforeach;
		...
		function render_calendar_events() {
			...
			$events = apply_filters( 'calendar_timezone', $events, $_POST[ 'timezone' ] );
